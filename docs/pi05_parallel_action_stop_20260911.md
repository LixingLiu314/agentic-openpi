...
## 9. v2正式启动确认（2026-09-11 11:54:56）

v2于11:49正式派发，训练commit为 `64bc1d9bb2ee72c09b82d009a6983fbd3f2f0db8`。首10步本地/cloud核验通过：CE11.2167512，global-only all32 flow0.128192954；S/B/A裁剪前梯度168.05524/21.22884/0.44532835。三类media已上传，首步输入和动作图实际检查通过。官方基座SHA、相同seed42新S、三组计数0和8个rank均已验证；这些启动指标不代表最终模型质量。

最后一次有界读取为17/5000，8个训练rank存活，源码清单逐项保持。steps3–17平均8.19114878秒/update，剩余纯训练11.3379小时，另加45分钟验证、存盘和最终native门槛，点估计2026-09-12约00:00；预计午夜至01:00左右，保留01:30单次跟进缓冲。实际时长受共享负载影响，若届时未完成再单次重估。后续不继续读取中间训练进度。

证据：`logs/pi05_parallel_action_stop_20260911/attempt_04/startup_handoff.json`、正式输出的`startup_verified.json`和`initialization.json`。没有push、Agilex部署或机器人动作。
