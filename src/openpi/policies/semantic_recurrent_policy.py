...
from openpi.models_pytorch.semantic_recurrent import SemanticRecurrentModel, VARIANT, SCHEMA_VERSION, EXPERIMENTS
...
def create_semantic_recurrent_policy(checkpoint, *, device="cpu", num_steps=10, allow_engineering=False):
    ...
    model=SemanticRecurrentModel(PI0Pytorch(Pi0Config(**options)).to(device),
    ...
    if (config.get("action_rank_coefficient"), config.get("action_rank_margin"), config.get("action_rank_max_pairs_per_rank")) != (.1 if experiment == "semantic_s_actionrank" else 0., .01, 4):
        raise ValueError("Changed action ranking contract")
    model.configure_semantics(config["semantic_vocabulary"], experiment)
