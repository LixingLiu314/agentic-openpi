"""Causal full-episode S validation plus fixed-frame action-condition controls."""
import numpy as np
import torch
...
def evaluate_recurrent(model, dataset, device, *, samples=128, draws=2, vocabulary=(),
    ...
    try:
        ...
        metrics.update(loss_subtask=float(totals[0] / totals[1].clamp_min(1)),
                       flow_generated_native14_normalized=float(totals[2] / totals[3].clamp_min(1)),
                       flow_generated_all32=float(totals[4] / totals[3].clamp_min(1)),
        ...
        metrics["selection_score"] = (.5 * (1 - metrics["normalized_exact_match"]) +
                                      ...
                                      5 * metrics["flow_generated_native14_normalized"])
        return metrics, rows
    finally:
