...
@torch.no_grad()
def evaluate_native(model, dataset, device, *, samples=128, draws=2, vocabulary=(), batch_size=2,
    ...
    for ep in episodes[rank::world]:
        ...
        streams.append(dict(episode=ep, indices=ordered, offset=0))
    totals = torch.zeros(9, dtype=torch.float64, device=device)
    rows = []
    try:
        while streams:
            active = streams[:batch_size]
            selected = [item["indices"][item["offset"]] for item in active]
            ...
            # Evaluate A on the fixed frames, with no subtask or history input.
                ...
                for name, conditions, offset in [("generated", texts, 2)] + (
                    ...
                    for draw in range(draws):
                        ...
                        if name == "generated":
                            ...
                            totals[5] += error[:, :25, :14].mean((1, 2)).sum()
                            totals[6] += error[..., [0,1,2,3,4,5,7,8,9,10,11,12]].mean((1, 2)).sum()
                            totals[7] += error[..., [6,13]].mean((1, 2)).sum()
            for item in active:
                item["offset"] += 1
        ...
        metrics.update(loss_subtask=float(totals[0] / totals[1].clamp_min(1)),
                       ...
                       flow_global_only_first25_native14=float(totals[5] / totals[3].clamp_min(1)),
                       flow_global_only_joints12=float(totals[6] / totals[3].clamp_min(1)),
                       flow_global_only_grippers2=float(totals[7] / totals[3].clamp_min(1)),
                       stateless=True, causal_interval_seconds=interval)
