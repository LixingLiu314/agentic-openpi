"""Independent provenance for official-initialized gradient comparisons."""
...
def build_run_config(args, cfg, dc, world):
    ...
    result.update(
        ...
        decoder=dataclasses.asdict(SubtaskDecoderConfig()), use_quantile_norm=dc.use_quantile_norm,
        ...
        initialization="official_pi05_base", inherited_training_updates=0, decoder_initialization_seed=args.seed,
        ...
        optimizer_transition="fresh S and A+B optimizers; zero inherited updates",
        gradient_contract="CE->S only; action->A plus permitted B; action never reaches S",
        ...
        training_source="audited official pi05_base plus seed42 fresh S; no trained parent",
        evaluation_source="validation_natural", prompt_contract="global task plus normalized native14 state and generated subtask",
        display_schema=1, display_set="official-pi05-backbone-s42-v1", experiment_family="official-backbone-gradient",
        num_train_steps=args.steps, dataset="eggplant_potato_gripper_binary", view_only=False,
