...
def main():
    ...
    policy=create_semantic_recurrent_policy(args.checkpoint,device=device)
    ...
    for ep in sorted(raw.episodes)[rank::world]:
        ...
        for draw in (0,1):
            ...
            rows.append(dict(episode=ep,layout=record["layout_visual"],draw=draw,kind="switched_goal_correct_actor",
                             expected_actor=switched_actor,subtask=correct_text,action_actor=actor,joint_displacement_LR=displacement))
