...
def prefix_actors(actions,state):
    return {str(n):dict(zip(["actor","displacement_LR"],dominant_arm(actions[:n],state))) for n in (15,25,50)}
...
def main():
    ...
    policy=create_decision_recurrent_policy(args.checkpoint,device=device)
        ...
        for draw in (0,1):
            ...
            for goal in ("original","switched"):
                ...
                rows.append(dict(episode=ep,layout=record["layout_visual"],draw=draw,kind="goal_"+goal,
                                 ...
                                 prefix_actors=prefix_actors(output["actions"],obs["state"]),action_actor=action_actor,joint_displacement_LR=displacement))
            switched=dict(obs, prompt=("Put the sweet potato into the box" if "eggplant" in obs["prompt"] else "Put the eggplant into the box"))
            switched_actor=("left" if original_actor=="right" else "right") if record["layout_visual"].startswith("opposite_sides") else original_actor
            correct_text=phase_actor(sample["subtask"])[0]+f" with the {switched_actor} arm"
            action=text_intervention(policy,switched,correct_text,noise)
            actor,displacement=dominant_arm(action,obs["state"])
            rows.append(dict(episode=ep,layout=record["layout_visual"],draw=draw,kind="switched_goal_correct_actor",
                             expected_actor=switched_actor,subtask=correct_text,prefix_actors=prefix_actors(action,obs["state"]),action_actor=actor,joint_displacement_LR=displacement))
            ...
            for condition,text in [("correct_actor",sample["subtask"]),("actor_removed",phase),
                ...
                rows.append(dict(episode=ep,layout=record["layout_visual"],draw=draw,kind=condition,
                                 expected_actor=original_actor,subtask=text,prefix_actors=prefix_actors(action,obs["state"]),action_actor=actor,
                                 joint_displacement_LR=displacement))
    ground_rows=[]
    if policy.model.grounded:
        from openpi.training.decision_data import load_assets,GroundingSamples
        _,points,_=load_assets(args.checkpoint/"assets/decision")
        grounding=GroundingSamples(dataset,points,split="val")
        with torch.no_grad():
            for index in range(rank,len(grounding.samples),world):
                batch,point=grounding.batch([index],device)
                context=policy.model.prepare_context(batch.observation,batch.global_prompts)
                ce,distance,scores=policy.model.decoder.localization_loss(context.memory,context.memory_mask,point)
                peak=int(scores.argmax(-1)[0]);x,y=((peak%16+.5)*14,(peak//16+.5)*14)
                r=grounding.samples[index];box=r["bbox"]
                ground_rows.append(dict(episode=r["episode"],frame=r["frame"],entity=r["entity"],distance_px=float(distance),
                    peak_xy=[x,y],peak_in_box=box[0]<=x<box[2] and box[1]<=y<box[3],heatmap_ce=float(ce)))
    if world>1:
        gathered=[None]*world;dist.all_gather_object(gathered,ground_rows)
        ground_rows=[r for part in gathered for r in part]
    ...
    if rank==0:
        ...
        for kind in sorted({r["kind"] for r in rows}):
            ...
            summary[kind]["executed_prefix"]={str(n):dict(correct=sum(r["prefix_actors"][str(n)]["actor"]==r["expected_actor"] for r in subset),
                 ambiguous=sum(r["prefix_actors"][str(n)]["actor"]=="ambiguous" for r in subset)) for n in (15,25,50)}
        ...
        report["grounding"]=dict(rows=ground_rows,count=len(ground_rows),
            mean_distance_px=float(np.mean([r["distance_px"] for r in ground_rows])) if ground_rows else None,
            scope="Reviewed validation keyframes only; patch-grid localization proxy, not grasp success")
