"""Official pi05 training with separate A/B gradient visibility and unchanged optimization."""
...
from openpi.models_pytorch.official_backbone_gradient import initialize_official, tensor_digest, VARIANT
...
from openpi.training.backbone_observability import GradientObserver, configure_run, log_event, OBSERVABILITY_VERSION
...
class Optimizers:
    def __init__(self, model, world):
        ...
        self.world = world
        self.gradient_observer = GradientObserver(model)
    ...
    def step(self, lr, *, observe=False):
        diagnostics = self.gradient_observer.measure() if observe else {}
...
def save_checkpoint(model, optimizers, args, config, step, best, counters, assets):
    ...
    if rank == 0:
        ...
        metadata = {"schema_version": 5, "stage": "official_backbone_grad", "variant": VARIANT,
...
def train(args):
    ...
    args.stage, args.overfit_samples = "official_backbone_grad", 0
    payload = [build_run_config(args, cfg, dc, world) if rank == 0 else None]
    if rank == 0:
        payload[0].update(
            observability_version=OBSERVABILITY_VERSION,
            gradient_diagnostics_every=10,
            gradient_diagnostics_scope="rank0 after DDP reduction, before unchanged joint A+B clipping",
            backbone_loss_source="train/action_flow_mse_all32; no independent B loss",
        )
        for name in ["scripts/train_official_backbone_gradient_observed.py",
                     "src/openpi/training/backbone_observability.py",
                     "scripts/continue_official_full_observed.py",
                     "scripts/verify_official_full_wandb_startup.py"]:
    ...
    if rank == 0 and not args.resume:
        write_json(args.output / "initialization.json", {
            ...
            "decoder_sha256":initial_s_hash, "decoder_seed":args.seed, "inherited_training_updates":0,
    ...
    loader = torch.utils.data.DataLoader(dataset, batch_sampler=sampler, collate_fn=collate_pinned_subtask,
    ...
    if rank == 0 and args.wandb:
        try:
            ...
            wb = wandb.init(entity="xiahy23-tsinghua-university", project="agentic-openpi-pi05-subtask",
                             name=f"Official | B-{args.mode} | s{args.seed}",
                             ...
                             tags=["display-v1", "engineering" if args.engineering_smoke else "research", "backbone-gradient", args.mode, "seed42"],
            ...
            raise RuntimeError("W&B initialization failed; refusing to silently run without requested metrics") from error
    ...
    def validate(step):
        result, rows = training.evaluate(model, validation, device, samples=args.eval_samples,
                                          batch_size=args.eval_batch_size, draws=args.eval_draws,
    ...
    for step in range(start, args.steps):
        ...
        norms = optimizers.step(lr, observe=rank == 0 and (step == 0 or (step + 1) % 10 == 0))
        optimizers.zero_grad()
