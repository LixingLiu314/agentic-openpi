"""Matched stateless/recurrent S, official fresh B/A, causal episode sampling."""
...
def save_checkpoint(model, optimizers, args, config, step, best, counters, assets, carry):
    ...
    if rank == 0:
        ...
        metadata = {"schema_version": 6, "stage": "recurrent_subtask", "variant": VARIANT,
...
def train(args):
    ...
    if not args.engineering_smoke and (args.steps not in (4000, 4500, 5000) or
    ...
    if args.mode != "frozen" or args.accumulation != 1:
        raise ValueError("First recurrent pair fixes frozen B and one time-major block per update")
    payload = [build_run_config(args, cfg, dc, world) if rank == 0 else None]
    if world > 1:
        dist.broadcast_object_list(payload, src=0)
    config = payload[0]
    config.update(variant=VARIANT, stage="recurrent_subtask",
                  display_set="official-pi05-recurrent-s42-v1", experiment_family="recurrent-subtask",
                  ...
                  checkpoint_selection="0.5*(1-normalized_EM)+0.5*(1-macro_F1)+5*native14_flow_MSE",
                  budget_reason="User authorized two models within approximately ten hours; matched measured budget")
    new_sources = ["scripts/train_recurrent_subtask.py", "src/openpi/models_pytorch/recurrent_subtask.py",
                   "src/openpi/training/recurrent_sequence.py", "src/openpi/training/recurrent_evaluation.py",
                   "src/openpi/policies/recurrent_subtask_policy.py", "scripts/serve_recurrent_subtask_piper.py"]
    ...
    initial = model
    model = RecurrentSubtaskModel(initial.base, SubtaskDecoderConfig(), recurrent=args.arm == "recurrent",
    ...
    if rank == 0 and args.wandb:
        try:
            ...
            wb = wandb.init(entity="xiahy23-tsinghua-university", project="agentic-openpi-pi05-subtask",
                             name=f"Official | S-{args.arm} | s{args.seed}",
                             ...
                             tags=["display-v1", "engineering" if args.engineering_smoke else "research", "recurrent-subtask", args.arm, "seed42"],
...
def parse_args():
    ...
    p.add_argument("--mode", choices=["frozen"], default="frozen")
    p.add_argument("--arm", choices=["stateless", "recurrent"], required=True)
