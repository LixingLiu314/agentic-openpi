...
from openpi.training.reach_arm_data import data_config, VARIANT, SCHEMA_VERSION, DISPLAY_SET, ANNOTATIONS_SHA256
...
def train(args):
    ...
    config.update(variant=VARIANT, stage="recurrent_subtask",
                  display_set=DISPLAY_SET, experiment_family="recurrent-reach-arm",
                  ...
                  budget_reason="User authorized this candidate only, standard 5000 updates",
    ...
    new_sources = ["scripts/train_reach_arm_candidate.py", "src/openpi/models_pytorch/recurrent_subtask.py",
    ...
    if rank == 0 and args.wandb:
        try:
            ...
            wb = wandb.init(entity="xiahy23-tsinghua-university", project="agentic-openpi-pi05-subtask",
                             name=f"Limited + recurrent + reach-arm | s{args.seed}",
