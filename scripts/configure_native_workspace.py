...
from openpi.training.native_subtask_wandb import AXIS


def save_workspace(root, *, display_set="official-pi05-native-n1-reach-arm-s42-v1", name="N1 native VLM action-stop s42", receipt=None):
    import wandb
    import wandb_workspaces.workspaces as ws
    import wandb_workspaces.reports.v2 as wr
    entity, project = "xiahy23-tsinghua-university", "agentic-openpi-pi05-subtask"
    receipt = receipt or root / "logs/pi05_native_n1_20260911/attempt_01/wandb_workspace.json"
    runs = list(wandb.Api().runs(f"{entity}/{project}", filters={"config.display_set": display_set, "config.display_schema": 1}))
    colors = {"limited": "#2563EB", "full": "#EA580C"}
    ...
    sections = [
        section("01 验证效果 · 优先看这三张", [
            plot("动作 Flow MSE ↓ | global-only, native14", "val/action_flow_mse_global_only_native14", "Normalized flow MSE (14 dims)"),
        ...
        section("02 学习过程 · train 与 val 分开", [
            plot("Action loss ↓ | train, global-only all32", "train/action_flow_mse_global_only_all32", "Flow MSE (32 dims)"),
            plot("Subtask CE ↓ | train", "train/subtask_ce", "Token cross entropy"),
            plot("Subtask CE ↓ | validation", "val/subtask_ce", "Token cross entropy"),
            plot("学习率 | B / A 共用 schedule", "optim/lr", "Learning rate"),
        ]),
        section("03 诊断 · 需要时展开", [
            plot("B 梯度范数 | native CE, clipping 前", "optim/grad_norm_b", "L2 norm"),
            plot("A 梯度范数 | flow, clipping 前", "optim/grad_norm_a", "L2 norm"),
            ...
            plot("B 梯度裁剪比例", "optim/clip_scale_b", "Scale (0–1)", (0, 1)),
            plot("A 梯度裁剪比例", "optim/clip_scale_a", "Scale (0–1)", (0, 1)),
        ], False),
    ...
    runset = ws.RunsetSettings(
        ...
        groupby=[], run_settings={r.id: ws.RunSettings(color=colors.get(r.config.get("variant"), "#64748B")) for r in runs},
        pinned_columns=["run:displayName", "config:variant", "config:seed", "summary:last_optimizer_step",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--display-set", default="official-pi05-native-n1-reach-arm-s42-v1")
    parser.add_argument("--name", default="N1 native VLM action-stop s42")
