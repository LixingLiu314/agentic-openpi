"""Larger microbatches and audited decoded inputs, preserving the action->B recipe."""
...
def train(args):
    ...
    if rank == 0:
        payload[0].update(
            ...
            display_schema=1, display_set="backbone-grad-largebatch-s42",
            experiment_family="backbone-gradient", num_train_steps=args.steps,
            dataset="eggplant_potato_gripper_binary", view_only=False,
            data_pipeline="rgb224_cache_v1; persistent workers; pinned memory; nonblocking transfer",
        ...
        for name in ["scripts/train_backbone_gradient.py", "scripts/run_backbone_gradient_pair.py",
                     ...
                     "scripts/train_backbone_gradient_largebatch.py",
                     "scripts/run_backbone_largebatch_retry.py",
                     "src/openpi/training/decoded_video_cache.py",
                     "packages/openpi-client/src/openpi_client/image_tools.py",
                     "scripts/build_piper_rgb_cache.py",
                     "scripts/verify_piper_rgb_cache.py",
                     "src/openpi/training/wandb_standard.py"]:
    ...
    if rank == 0 and args.wandb:
        try:
            ...
            wb = wandb.init(entity="xiahy23-tsinghua-university", project="agentic-openpi-pi05-subtask",
                             name=f"B-{args.mode} b{args.batch_size} | s{args.seed}",
