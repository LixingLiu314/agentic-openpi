"""Official fresh parallel S/A: CE trains B+S; action trains A only."""
...
from openpi.models_pytorch.parallel_subtask import ParallelSubtaskModel, VARIANT, SCHEMA_VERSION, DISPLAY_SET
...
from openpi.training.parallel_evaluation import evaluate_recurrent
...
from openpi.training.parallel_wandb import configure_run, log_event
...
class Optimizers:
    def __init__(self, model, world):
        ...
        self.parameters = {"subtask": model.subtask_parameters(), "action": model.action_parameters(),
...
def save_checkpoint(model, optimizers, args, config, step, best, counters, assets, carry):
    ...
    if rank == 0:
        ...
        metadata = {"schema_version": SCHEMA_VERSION, "stage": "parallel_subtask", "variant": VARIANT,
...
def train(args):
    ...
    args.stage, args.overfit_samples = "parallel_subtask", 0
    if (args.mode,args.arm,args.unroll,args.condition_dropout) != ("action_stop","recurrent",4,0.0):
        raise ValueError("Parallel action-stop recipe mismatch")
    payload = [build_run_config(args, cfg, dc, world) if rank == 0 else None]
    if world > 1:
        dist.broadcast_object_list(payload, src=0)
    config = payload[0]
    config.update(variant=VARIANT,stage="parallel_subtask",display_set=DISPLAY_SET,
        experiment_family="parallel-action-stop",memory_tokens=4,
        memory_input="observation-only S memory; no session memory or subtask text reaches A",
        sampling="persistent episode streams; time-major; truncated gradients each optimizer block",
        ...
        gradient_contract="CE->S+B; flow->A only; every prefix K/V detached for A",
        ...
        optimizer_transition="fresh disjoint S/B/A AdamW owners; one update per parameter",
        ...
        evaluation_version="parallel_global_only_v1; same reach-arm causal sample protocol",
    ...
    initial = model
    model = ParallelSubtaskModel(initial.base, SubtaskDecoderConfig(), recurrent=True,
    ...
    if rank == 0 and not args.resume:
        write_json(args.output / "initialization.json", {
            ...
            "decoder_sha256":initial_s_hash, "complete_decoder_sha256":initial_complete_s_hash, "arm":args.arm, "decoder_seed":args.seed, "inherited_training_updates":0,
    ...
    if rank == 0 and args.wandb:
        try:
            ...
            wb = wandb.init(entity="xiahy23-tsinghua-university", project="agentic-openpi-pi05-subtask",
                             name=f"Parallel action-stop | s{args.seed}",
                             ...
                             tags=["display-v1", "engineering" if args.engineering_smoke else "research", "reach-arm", "parallel", "action-stop", "seed42"],
    ...
    for step in range(start, args.steps):
        ...
        for micro in range(args.accumulation):
            ...
            with sync:
                result = model(batch, carry=carry, drop_condition=dropped)
                carry = result["carry"]
        ...
        if completed == 10 and rank == 0 and args.wandb and not args.engineering_smoke:
            subprocess.run([".stage1_staging/wandb_display_env/bin/python","scripts/verify_parallel_startup.py",
