...
def save_workspace(root, *, display_set="backbone-grad-m3-s42", name="Pi05 Backbone gradients s42 v1", receipt=None):
    import wandb
    ...
    colors = {"decision_prefix":"#2563EB","decision_grounded":"#EA580C"}
    ...
    sections += [section("06 决策 · 选臂与选物分别观察", [
        plot("前25步 native14 Flow MSE", "decision/first25_native_mse", "Flow MSE"),
        plot("换臂对比样本数", "decision/rank_arm_pairs", "Pairs per global update"),
        plot("换物对比样本数", "decision/rank_object_pairs", "Pairs per global update"),
        plot("换臂 hinge", "decision/rank_arm_hinge", "Hinge"),
        plot("换物 hinge", "decision/rank_object_hinge", "Hinge"),
        plot("前缀加权 action flow", "decision/action_weighted_flow", "Weighted MSE"),
    ]), section("07 目标定位 · 仅第二组有该监督", [
        plot("训练定位热图 CE", "decision/grounding_ce", "Cross entropy"),
        plot("训练定位像素误差", "decision/grounding_distance_px", "Pixels on 224 image"),
    ], True, 2)]
    ...
    runset = ws.RunsetSettings(
        ...
        groupby=[], run_settings={r.id: ws.RunSettings(color=colors.get(r.config.get("experiment"), "#64748B")) for r in runs},
...
if __name__ == "__main__":
    ...
    parser.add_argument("--display-set", default="backbone-grad-m3-s42")
    parser.add_argument("--name", default="Pi05 Backbone gradients s42 v1")
