...
def save_workspace(root, *, display_set="backbone-grad-m3-s42", name="Pi05 Backbone gradients s42 v1", receipt=None):
    ...
    sections = [
        ...
        section("02 学习过程 · train 与 val 分开", [
            plot("Action loss ↓ | train · A/B 共用", "train/action_flow_mse_all32", "Shared action Flow MSE (32 dims)"),
        ...
        section("03 B 梯度 · Full B 起提供独立指标", [
            plot("A 梯度范数 | 联合裁剪前", "optim/grad_norm_a", "L2 gradient norm"),
            plot("整个 B 梯度范数 | action loss 回传", "optim/grad_norm_b", "L2 gradient norm"),
            plot("视觉 B 梯度 | SigLIP 与视觉投影", "optim/grad_norm_b_vision", "L2 gradient norm"),
            plot("语言 B 梯度 | 含 token embedding", "optim/grad_norm_b_language", "L2 gradient norm"),
        ], True, 2),
        section("04 常规诊断 · 需要时展开", [
            ...
            plot("有梯度的 B 参数张量比例", "optim/b_grad_tensor_fraction", "Fraction with allocated gradient", (0, 1)),
        ], False),
        section("05 速度 · 不含验证及存盘", [
    ...
    runset = ws.RunsetSettings(
        ...
        groupby=[], run_settings={f"{mode}_seed42_v1": ws.RunSettings(color=color) for mode, color in colors.items()},
