...
from openpi.models_pytorch.semantic_recurrent import SemanticRecurrentModel, VARIANT, SCHEMA_VERSION, DISPLAY_SET, EXPERIMENTS, stable_training_frames
...
def train(args):
    ...
    config.update(variant=VARIANT, stage="recurrent_subtask", experiment=args.experiment,
                  display_set=DISPLAY_SET, experiment_family="recurrent-semantic-pair",
                  ...
                  semantic_weight=4.0, action_rank_coefficient=.1 if args.experiment == "semantic_s_actionrank" else 0.,
    ...
    stable_frames = stable_training_frames(raw.hf_dataset)
    config["action_rank_stability"] = "same training label across past7/current/future49 frames, clipped at episode ends"
    config["action_rank_stable_train_frames"] = len(stable_frames)
    config["engineering_condition_fixture"] = args.engineering_smoke and args.experiment == "semantic_s_actionrank"
    ...
    initial = model
    model = SemanticRecurrentModel(initial.base, SubtaskDecoderConfig(), recurrent=args.arm == "recurrent",
                                  seed=args.seed, unroll=args.unroll)
    del initial
    model.configure_semantics(vocabulary, args.experiment)
    ...
    for step in range(start, args.steps):
        ...
        for micro in range(args.accumulation):
            ...
            with sync:
                ...
                with fixture_context:
                    result = model(batch, carry=carry, drop_condition=dropped, rank_offset=step, rank_stable_mask=rank_stable)
                carry = result["carry"]
            ...
            totals[5] += result["loss_subtask"].detach()
        ...
        log({"event":"train", "step":completed, "loss_subtask":float(totals[0]), "loss_action":float(totals[1]),
             ...
             "action_rank_hinge":float(totals[6]), "rank_pairs":int(totals[7]), "lr":lr, "grad_norms":norms,
