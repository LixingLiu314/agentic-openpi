...
def save_workspace(root, *, display_set="official-pi05-parallel-action-stop-s42-v1", name="Pi05 Parallel Action-stop s42 v1", receipt=None):
    import wandb
    ...
    sections = [
        section("01 验证效果 · 优先看这三张", [
            plot("动作 Flow MSE ↓ | validation, normalized native14", "val/action_flow_mse_global_only_native14", "Normalized flow MSE (14 dims)"),
        ...
        section("02 学习过程 · train 与 val 分开", [
            plot("Action loss ↓ | train, all32", "train/action_flow_mse_global_only_all32", "Flow MSE (32 dims)"),
        ...
        section("03 诊断 · 需要时展开", [
            ...
            plot("B 梯度范数 | CE only", "optim/grad_norm_b", "L2 norm"),
            plot("B 采样相对参数更新", "optim/update_sampled_relative_b", "Sampled relative L2 change"),
        ], False),
    ...
    sections.insert(3, section("03b 阶段与选臂 · validation", [
        ...
        plot("A 梯度范数 | clipping 前", "optim/grad_norm_a", "L2 norm"),
        plot("B 梯度范数：subtask CE | clipping 前", "optim/grad_norm_b", "L2 norm"),
    ], False))
    ...
    runset = ws.RunsetSettings(
        ...
        groupby=[], run_settings={"action_stop_seed42_v1": ws.RunSettings(color="#2563EB")},
...
if __name__ == "__main__":
    ...
    parser.add_argument("--display-set", default="official-pi05-parallel-action-stop-s42-v1")
    parser.add_argument("--name", default="Pi05 Parallel Action-stop s42 v1")
