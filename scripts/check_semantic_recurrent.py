...
def main():
    ...
    policy = create_semantic_recurrent_policy(args.checkpoint, device=args.device,
    ...
    if args.allow_engineering:
        ...
        ce,ordinary = model.semantic_loss(sequence.target_ids,sequence.target_mask,projected,mask)
        ce.backward()
        ...
        model.zero_grad(set_to_none=True)
        from openpi.models_pytorch.semantic_recurrent import ranking_loss
        positives=list(sequence.labels)
        negatives=[model.negative_labels[s][0] for s in positives]
        positive_errors=model.action_errors(context,positives,sequence.actions,noise=fixed_noise,time=fixed_time)
        negative_errors=model.action_errors(context,negatives,sequence.actions,noise=fixed_noise,time=fixed_time)
        ranking=ranking_loss(positive_errors[:,:,:14].mean((1,2)),negative_errors[:,:,:14].mean((1,2)))
        ...
        model.zero_grad(set_to_none=True)
        # Activate the branch in the actual trainer forward even before S learns words.
        # This is an engineering fixture only; formal generation is never replaced.
        ...
        with patch.object(model.decoder,"generate_projected",return_value=fixture):
            ...
            assert result["rank_pairs"] == (4 if model.experiment=="semantic_s_actionrank" else 0)
            (result["loss_subtask"]+result["loss_action"]).backward()
            assert torch.isfinite(result["loss_subtask"]+result["loss_action"])
        model.zero_grad(set_to_none=True)
        gradient_contract = dict(weighted_ce_only_S=True,ce_only_S=True,recurrence_gradient_nonzero=True,
