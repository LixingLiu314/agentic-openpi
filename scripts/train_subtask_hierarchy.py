...
def validate_transition(args, metadata):
    ...
    if args.stage == "m3":
        if metadata["completed_steps"] != metadata["config"]["steps"]:
            raise ValueError("M3 must inherit the completed M2 warmup to preserve the matched action budget")
    ...
    sources = [
        ...
        *[
            ...
            for name in [
                ...
                "training/subtask_curriculum.py",
...
def train(args):
    ...
    for step in range(start, args.steps):
        ...
        if completed % args.eval_every == 0 or completed in {args.steps, args.stop_after}:
            ...
            eligible = selection_eligible(args.stage, completed, args.steps, counters["action"])
            improved = eligible and (best is None or score < best["selection_score"])
            log({"event": "selection_eligibility", "step": completed, "eligible": eligible, "score": score})
            if improved:
