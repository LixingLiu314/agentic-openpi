...
variants in ``src/openpi/training/config.py``:
  ...
  - basic    : pi05_aloha_food_baseline      (no extra inputs)
  - traj     : pi05_aloha_food_traj          (prompt suffixed with ", traj: ...")
  - subtask  : pi05_aloha_food_subtask       (prompt suffixed with ", subtask: ...")
  - triple_cot : semi-block prompt with task + subtask + trajectory CoT
  - subgoal  : pi05_aloha_food_subgoal       (extra ``subgoal_images`` field)
